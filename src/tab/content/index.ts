...
(async (_global, globalThis) => {
    ...
    const bridge = createBridge({ sendPrefix: '2P', listenPrefix: '2C', cloneInto });
    bridge.init(COMM_ID);

    bridge.setMessageListener((request: any, sendResponse) => {
        if (V) console.log('content: forwarding request', request);
        runtime.sendMessage({ ...request, method: 'userscripts' } , (response) => {
